...
use util::sdk_resources::{SdkSourceConfig, find_builtin_sdk_config};
use util::warning;
...
pub fn fuzzy_match_version(entries: &[VersionEntry], version_input: &str) -> Result<ResolvedVersion> {
    ...
    let matching: Vec<&VersionEntry> = entries.iter().filter(|e| e.full_version.starts_with(&prefix)).collect();
