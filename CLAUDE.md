...
9. **hotfix：`remove_sdk_path` filter 类型修复**（`env/unix.rs`）— 第 6 条 unix.rs 重构时，`remove_sdk_path` 的 `split().filter()` 闭包写成 `|p| p != &expanded_target && p != target`，`p: &str` 与 `&expanded_target: &String` 没有直接 `impl PartialEq`，deref coercion 在 `!=` 不触发，类型错误级联报在 `p != target` 上（`&str == str`）。改 `expanded_target.as_str()` 取 `&str`。**Windows `cargo build` 不编译 `cfg(unix)` 模块所以漏检，v0.2.5 CI build job 在 Linux 上失败（release 未发布，仅 tag 创建），删 tag 重发修复。详见「已知问题与注意事项」cfg(unix) 盲区条**
...
- **cfg(unix) 代码在 Windows 上不编译，类型错误会漏到 Linux 才暴露**：`env/unix.rs` 整个模块 `#[cfg(unix)]`，Windows 上 `cargo build`/`clippy` 完全跳过，类型不匹配在 WSL/Linux 编译时才报。改 `unix.rs` 后**必须**用 `cargo check --target x86_64-unknown-linux-gnu`（仅类型检查，但会被 aws-lc-rs 的 C 交叉编译 build script 阻塞，找不到 `x86_64-linux-gnu-gcc` 时只能确认 Rust 类型错误是否消失，不能完整通过）或直接在 WSL 里 `cargo build` 验证。**filter 闭包比较 `&str` 与 `String` 时用 `.as_str()` 取 `&str`，不要用 `&` 取 `&String`**——`&str != &String` 没有直接 `impl PartialEq`，deref coercion 在 `!=` 上下文不按预期触发，会把类型错误级联报在同一行的其他比较上（2026-07-07 重构 `remove_sdk_path` 时踩过：`p != &expanded_target && p != target` 报 `&str == str` 指向第二个比较，实际根因是第一个比较的 `&String`）。原版 `28b2c43` 用 `|&p|` 解构是因为 item 是 `&&str`（从 `Vec.iter()` 来）；重构后 `split()` 直接 filter，item 是 `&str`，不能再 `|&p|` 解构（str unsized），改用 `|p|` + `.as_str()`
