...
1. 诚实来讲,  sdkm 和 mise 几乎是同类工具，目前生态，功能都不如mise成熟。但sdkm当下只专注于做好一件事："**快速、透明、跨平台体验一致的版本切换。**"
...
> **⚠️ Windows 用户**：`init`/`switch` 需以管理员权限启动终端运行（写入系统环境变量 `HKEY_LOCAL_MACHINE` + 创建符号链接）。
